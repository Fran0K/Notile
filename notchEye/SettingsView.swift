...
struct GeneralTab: View {
    ...
    @AppStorage("testMode") private var testMode: Bool = false
    ...
    var body: some View {
        ...
        Form {
            ...
            Section("测试模式") {
                Toggle("启用（每 30 秒触发一次）", isOn: $testMode)
                if testMode {
                    Text("注意：测试模式仅在 Debug 构建或手动开启时生效")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
            }
...
struct DisplayTab: View {
    let store = (NSApp.delegate as? AppDelegate)?.timerManager.store ?? ReminderStore()
...
//            }
    ...
    private func chooseLottieFileForDraft() {
        ...
        do {
            let finalName = try copyMediaWithThumbnail(
