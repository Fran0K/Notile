...
extension Notification.Name {
    static let notechTriggerNow = Notification.Name("notechTriggerNow")
    static let notechItemDeleted = Notification.Name("notechItemDeleted")
    static let notechDidExpand = Notification.Name("notechDidExpand")
    static let notechDidCollapse = Notification.Name("notechDidCollapse")
}
...
final class TimerManager {
    ...
    private func scheduleReminder(item: ReminderItem) {
        ...
        let interval: TimeInterval = testMode ? 30 : interval(for: item)
        #else
        let interval: TimeInterval = interval(for: item)
        #endif
    ...
    private func expand(item: ReminderItem) {
        ...
        isExpanded = true
        NotificationCenter.default.post(name: .notechDidExpand, object: nil)
        collapseTimer?.invalidate()
    ...
    private func collapse() {
        ...
        isExpanded = false
        activeItemId = nil
        NotificationCenter.default.post(name: .notechDidCollapse, object: nil)
    ...
    private func observeTriggerNow() {
        triggerCancellable = NotificationCenter.default
            .publisher(for: .notechTriggerNow)
